...
export default function InterviewList({ excludeId }) {
  const { data: interviews = [] } = useQuery({
    queryKey: ['interviews-list', excludeId],
    queryFn: () => base44.entities.Article.filter({ status: 'published', is_interview: true }, '-published_date', 10),
  });

  const others = interviews.filter(a => a.id !== excludeId).slice(0, 4);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="font-display font-black text-lg">More Interviews</h3>
      </div>
      {others.map((article) => (
        <Link key={article.id} to={articleUrl(article)} className="group flex gap-4 bg-white border-2 border-border rounded-2xl p-4 hover:border-accent/30 hover:shadow-md transition-all">
          ...
          <div className="flex flex-col justify-between min-w-0">
            <div>
              <span className="text-xs font-black text-accent uppercase tracking-wide">{article.category.replace(/_/g, ' ')}</span>
              <h4 className="font-display font-bold text-sm leading-snug mt-0.5 group-hover:text-accent transition-colors">
                {article.title}
              </h4>
              {article.interview_quote && (
                <p className="text-xs text-muted-foreground mt-1 line-clamp-2 leading-relaxed">
                  "{article.interview_quote}"
                </p>
              )}
            </div>
            <span className="text-xs font-bold text-accent mt-2 group-hover:underline">Read Interview →</span>
          </div>
        </Link>
      ))}
    </div>
