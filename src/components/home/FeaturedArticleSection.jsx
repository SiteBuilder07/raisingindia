  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Side Stories — navy brand card */}
        <div className="bg-primary rounded-3xl p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 mb-5">
              <span className="text-xs font-black uppercase tracking-widest text-primary bg-accent px-3 py-1 rounded-full">
                📰 News You Can Use
              </span>
            </div>
            <div className="space-y-4">
              {sideArticles.slice(0, 3).map((a) => (
                <Link key={a.id} to={`/Article?id=${a.id}`} className="block group">
                  ...
                  {a.summary && (
                    <p className="text-primary-foreground/70 text-xs mt-1 line-clamp-2 font-medium leading-relaxed">
                      {a.summary}
                    </p>
                  )}
                </Link>
          ...
          <Link to="/Categories">
            <button className="mt-6 w-full bg-accent text-white font-bold text-sm py-3 rounded-2xl hover:bg-accent/90 transition-colors">
              Explore More Issues →
            </button>
          </Link>
        </div>
